...
public class AnimatorBuilder
{
	...
	public AnimatorBuilder applyScale(View viewToScale, float scaleX, float scaleY, Interpolator interpolator)
	{
		...
		boolean hasScaleAnimation = hasAnimation(viewToScale, AnimatorBundle.TypeAnimation.SCALEX,
				AnimatorBundle.TypeAnimation.SCALEXY);
		...
		if (scaleX == scaleY && startScaleX == startScaleY)
		{
			...
			AnimatorBundle animatorScaleXY = AnimatorBundle.create(AnimatorBundle.TypeAnimation.SCALEXY, viewToScale,
					interpolator, startScaleX, scaleX);
		...
		{
			AnimatorBundle animatorScaleX = AnimatorBundle.create(AnimatorBundle.TypeAnimation.SCALEX, viewToScale,
					interpolator, startScaleX, scaleX);
			AnimatorBundle animatorScaleY = AnimatorBundle.create(AnimatorBundle.TypeAnimation.SCALEY, viewToScale,
					interpolator, startScaleY, scaleY);
			...
			Interpolator interpolator)
	{
		...
		AnimatorBundle animatorTranslationX = AnimatorBundle.create(AnimatorBundle.TypeAnimation.TRANSLATIONX,
				viewToTranslate, interpolator, startTranslationX, translateX);
		AnimatorBundle animatorTranslationY = AnimatorBundle.create(AnimatorBundle.TypeAnimation.TRANSLATIONY,
				viewToTranslate, interpolator, startTranslationY, translateY);
	...
	public AnimatorBuilder applyFade(View viewToFade, float fade, Interpolator interpolator)
	{
		...
		addAnimator(
				AnimatorBundle.create(AnimatorBundle.TypeAnimation.FADE, viewToFade, interpolator, startAlpha, fade));
	...
	public AnimatorBuilder applyVerticalParallax(View viewToParallax, float velocityParallax)
	{
		...
		addAnimator(AnimatorBundle.create(AnimatorBundle.TypeAnimation.PARALLAX, viewToParallax, null, 0f,
				-velocityParallax));
		...
		for (AnimatorBundle animator : animators)
		{
			if (!mSetAnimatorBundles.add(animator))
			{
				throw new IllegalArgumentException(
						"Animation " + animator.mTypeAnimation + " already added to this view");
			}
		}
	}
	...
	private void adjustTranslation(View viewAnimated)
	{
		...
		{
			animatorTranslationX.mDelta = animatorTranslationX.mDelta
					+ (animatorTranslationX.mView.getWidth() * (animatorScaleX.mDelta / 2f));
		}

		if (animatorTranslationY != null && animatorScaleY != null)
		{
			animatorTranslationY.mDelta = animatorTranslationY.mDelta
					+ (animatorTranslationY.mView.getWidth() * (animatorScaleY.mDelta / 2f));
		}
	...
	protected void animateOnScroll(float boundedRatioTranslationY, float translationY)
	{
		...
		for (AnimatorBuilder.AnimatorBundle animatorBundle : mSetAnimatorBundles)
		{
			...
			float interpolatedTranslation = animatorBundle.mInterpolator == null ? boundedRatioTranslationY
					: animatorBundle.mInterpolator.getInterpolation(boundedRatioTranslationY);
	...
	public static class AnimatorBundle
	{
		...
		public static AnimatorBundle create(AnimatorBundle.TypeAnimation typeAnimation, View view,
				Interpolator interpolator, float fromValue, float toValue)
		{
