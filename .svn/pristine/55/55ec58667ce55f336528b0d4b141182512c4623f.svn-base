...
public class MaterialTab implements View.OnTouchListener
{
	...
	private boolean isActivePersists;
	...
	public MaterialTab setText(Context context, CharSequence text)
	{
		if (hasIcon)
		{
		...
		this.text.setTypeface(Typeface.createFromAsset(context.getAssets(), "OpenSans-Regular.ttf"));
		return this;
	}
	...
	public void disableTab()
	{
		...
		if (text != null)
		{
			this.text.setTextColor(
					Color.argb(0xff, Color.red(textColor), Color.green(textColor), Color.blue(textColor)));
		}
	...
	public boolean isActivePersists()
	{
		return isActivePersists;
	}

	public void setActivePersists(final boolean activePersists)
	{
		isActivePersists = activePersists;
	}
	...
	public boolean onTouch(View v, MotionEvent event)
	{
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN)
		{
			if (!deviceHaveRippleSupport())
			{
				completeView.setBackgroundColor(
		...
		if (event.getAction() == MotionEvent.ACTION_UP)
		{
			...
			else
			{
				...
				this.background.reveal(lastTouchedPoint.x, lastTouchedPoint.y,
						Color.argb(0x80, Color.red(accentColor), Color.green(accentColor), Color.blue(accentColor)), 0,
						REVEAL_DURATION, new Animator.AnimatorListener()
						{
							@Override
							...
							{
								background.reveal(lastTouchedPoint.x, lastTouchedPoint.y, primaryColor, 0,
										HIDE_DURATION, null);
							}
	...
	private void setIconAlpha(int paramInt)
	{
		...
		this.icon.setColorFilter(Color.argb(paramInt, Color.red(this.iconColor), Color.green(this.iconColor),
				Color.blue(this.iconColor)));
	}
