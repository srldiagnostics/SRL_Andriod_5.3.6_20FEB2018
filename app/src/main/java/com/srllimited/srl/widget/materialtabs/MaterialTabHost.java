...
public class MaterialTabHost extends RelativeLayout implements View.OnClickListener
{
	...
	public MaterialTabHost(final Context context, final AttributeSet attrs, final int defStyleAttr,
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr)
	{
		...
		if (attrs != null)
		{
			...
			try
			{
				...
				primaryColor = a.getColor(R.styleable.MaterialTabHost_materialTabsPrimaryColor,
				...
				textSize = a.getInteger(R.styleable.MaterialTabHost_materialTabHostsTextSize, 10);
				scrollable = a.getBoolean(R.styleable.MaterialTabHost_materialTabHostScrollable, false);
			}
			finally
			{
				a.recycle();
	...
	public void setSelectionPersists(final boolean selectionPersists)
	{
		...
		{
			tab.setActivePersists(isSelectionPersists());
		}
	}
	...
	public void setSelectedNavigationItem(int position)
	{
		...
		else
		{
			...
			for (int i = 0; i < tabs.size(); i++)
			{
				...
				if (isSelectionPersists && i == position && tab.isSelected())
				{
					break;
				}
			...
			if (!isTablet)
			{
				for (int i = 0; i < tabs.size(); i++)
				{
					...
					if (i == 0)
					{
						...
						view.setMinimumWidth((int) (5 * density));
						layout.addView(view);
					}
