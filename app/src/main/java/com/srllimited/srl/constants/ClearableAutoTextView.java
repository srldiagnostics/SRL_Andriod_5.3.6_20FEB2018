...
public class ClearableAutoTextView extends AutoCompleteTextView
		implements OnTouchListener, OnFocusChangeListener, TextWatcher, OnKeyListener
{
	...
	private Handler mUiHandler = new Handler()
	{
		...
		public void handleMessage(Message msg)
		{
			...
			{
				AdapterAutoComplete autoCompleteAdapter = new AdapterAutoComplete(mContext, R.layout.auto_complete_item,
						result);
	...
	public ClearableAutoTextView(Context context, AttributeSet attrs) throws Exception
	{
		...
		if (google_places_api != null)
			mGooglePlacesApiKey = google_places_api.toString();
		if (input_key != null)
			mInputKey = input_key.toString();

		if (mAutocompleteUrl == null || mAutocompleteUrl.length() == 0)
		{
			if (mGooglePlacesApiKey == null)
				throw new Exception(
						"If autocomplete url is null then it uses Google places auto complete & so, you must specify google_places_api_key attribute in XML");
			mAutocompleteUrl = GoogleAutoCompleteParser.getGooglePlacesUrl(mGooglePlacesApiKey);
			mParser = new GoogleAutoCompleteParser();
		}
		else
		{
			if (input_key == null || input_key.length() == 0)
				throw new Exception("You must specify input key to send to autocomplete url");
		}
	...
	private void getLatLngFromAddress(String address)
	{
		...
		GetLatLngUtil.getLatLng(mContext, address, new GetLatLngUtil.GetLatLngResult()
		{
			...
			{
				if (mListener != null)
					mListener.onReceiveLocationInformation(lat, lng);
			}
		...
		public View getView(final int position, View convertView, ViewGroup parent)
		{
			...
			v.setOnClickListener(new View.OnClickListener()
			{
				...
				public void onClick(View v)
				{
					...
					if (mListener != null)
						mListener.onItemSelection(getItem(position));
					clearFocus();
					if (mAutocompleteUrl.contains(GoogleAutoCompleteParser.PLACES_API_BASE))
